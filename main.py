...
def translate(word):
    ...
    """
    s = openurl(word)
    # hush给的kivy的python版本是3.5的，所以这里要把bytes转换一下
    content = json.loads(s.decode('utf-8'))
    try:
        str = ''
...
class TestApp(App):
    ...
    def build(self):
        self.config_window()
        self.icon = 'spider.png'
        self.title = 'Jay\'s Spider Dict'
        root = self.setup_ui()
        return root
    ...
    def setup_ui(self):
        ...
        # btn1 = Button(text='X', size=(800, 200), size_hint=(None, None))
        # btn1.background_normal = 'zelda.png'
        # #btn1.background_active = ''
        ...
        input.background_normal = 'spider.png'
        layout.add_widget(input)

        result = TextInput()
        # log("result", result, "type", type(result))
    ...
    def check(self, input):
        ...
        means = translate(input.text)
        # 我们在这里可以设置 result 输入框的文本
        self.result.text = means
